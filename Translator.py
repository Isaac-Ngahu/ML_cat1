...
async def main(text): #the function that runs the translation
    ...
    if lang == "sw":
        translated_to_en = await translate_text(text, "sw", "en")
        print(f"Kiswahili text: {text} \nTranslated English Text: {translated_to_en}") #Outputs the translated text
    elif lang == "en":
        translated_to_sw = await translate_text(text, "en", "sw")
        print(f"English text: {text} \nTranslated Kiswahili Text: {translated_to_sw}") #Outputs the translated text
